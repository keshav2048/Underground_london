...
graph1 = AdjacencyListGraph(len(vertices), False, True)  # Creates the graph object using the Adjacency list library
for edge in edges:
    try:
        graph1.insert_edge(vertices.index(edge[0]), vertices.index(edge[1]), edge[2])
    except RuntimeError:  # If the edge is already present we just choose to ignore it
        pass
"""Using the kruskal algorithm to make the MST graph."""
mini = kruskal(graph1)

"""Identify edges that can be removed without disconnecting the network."""
removable_edges = []
for u in range(graph1.get_card_V()):
    for edge in graph1.get_adj_list(u):
        v = edge.get_v()
        # check where the edge is within the MST or not.
        if not mini.has_edge(u, v) and (min(u, v), max(u, v)) not in removable_edges:
            removable_edges.append((vertices[u], vertices[v],))
